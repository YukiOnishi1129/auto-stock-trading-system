...
var UserWhere = struct {
	...
	EmailVerified: whereHelpernull_Time{field: "`users`.`emailVerified`"},
...
var UserRels = struct {
	UserIdAccounts string
	UserIdSessions string
}{
	UserIdAccounts: "UserIdAccounts",
	UserIdSessions: "UserIdSessions",
}

// userR is where relationships are stored.
type userR struct {
	UserIdAccounts AccountSlice `boil:"UserIdAccounts" json:"UserIdAccounts" toml:"UserIdAccounts" yaml:"UserIdAccounts"`
	UserIdSessions SessionSlice `boil:"UserIdSessions" json:"UserIdSessions" toml:"UserIdSessions" yaml:"UserIdSessions"`
}
...
func (r *userR) GetUserIdAccounts() AccountSlice {
	if r == nil {
		return nil
	}
	return r.UserIdAccounts
}

func (r *userR) GetUserIdSessions() SessionSlice {
	if r == nil {
		return nil
	}
	return r.UserIdSessions
}
...
var (
	userAllColumns            = []string{"id", "name", "email", "emailVerified", "image", "created_at", "updated_at", "deleted_at"}
	userColumnsWithoutDefault = []string{"id", "name", "email", "emailVerified", "image", "deleted_at"}
...
// UserIdAccounts retrieves all the account's Accounts with an executor via userId column.
func (o *User) UserIdAccounts(mods ...qm.QueryMod) accountQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`accounts`.`userId`=?", o.ID),
	)

	return Accounts(queryMods...)
}

// UserIdSessions retrieves all the session's Sessions with an executor via userId column.
func (o *User) UserIdSessions(mods ...qm.QueryMod) sessionQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`sessions`.`userId`=?", o.ID),
	)

	return Sessions(queryMods...)
}

// LoadUserIdAccounts allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserIdAccounts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`accounts`),
		qm.WhereIn(`accounts.userId in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.UserIdAccounts = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserId {
				local.R.UserIdAccounts = append(local.R.UserIdAccounts, foreign)
				if foreign.R == nil {
					foreign.R = &accountR{}
				}
...
// LoadUserIdSessions allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserIdSessions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`sessions`),
		qm.WhereIn(`sessions.userId in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.UserIdSessions = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &sessionR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserId {
				local.R.UserIdSessions = append(local.R.UserIdSessions, foreign)
				if foreign.R == nil {
					foreign.R = &sessionR{}
				}
				foreign.R.UserIdUser = local
...
// AddUserIdAccounts adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserIdAccounts.
// Sets related.R.UserIdUser appropriately.
func (o *User) AddUserIdAccounts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Account) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			UserIdAccounts: related,
		}
	} else {
		o.R.UserIdAccounts = append(o.R.UserIdAccounts, related...)
	}
...
// AddUserIdSessions adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserIdSessions.
// Sets related.R.UserIdUser appropriately.
func (o *User) AddUserIdSessions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Session) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserId = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			UserIdSessions: related,
		}
	} else {
		o.R.UserIdSessions = append(o.R.UserIdSessions, related...)
	}
